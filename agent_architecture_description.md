# Descripción de la Arquitectura del Agente Conversacional Autónomo (Modelo/Cliché)

## 1. Concepto Central: Agente Autónomo Basado en Conversación y Acción

El núcleo de esta arquitectura es un **agente conversacional autónomo**, que sirve como un modelo o cliché para la creación de agentes especializados. Este agente interactúa con el usuario en lenguaje natural para comprender un objetivo o tarea. Luego, de forma autónoma:
...
Este ciclo permite al agente manejar tareas complejas que requieren múltiples interacciones con sistemas externos, manteniendo el contexto a lo largo del proceso. Este modelo se puede adaptar a diferentes dominios y herramientas.

## 2. Diseño del Prompt (`TEMPLATE`) para la Especialización

El prompt principal (la variable `TEMPLATE` en `main.py`) es crucial para instruir al LLM sobre su comportamiento y es la pieza clave para adaptar el agente a nuevos dominios. Para especializar el agente:
...
**Nota:** La estructura de este agente se ha reciclado del agente MongoDB. Puedes encontrar el repositorio del agente MongoDB aquí: [https://github.com/florinato/agenteMongoDB](https://github.com/florinato/agenteMongoDB)

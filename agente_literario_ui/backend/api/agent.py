...
from agente_literario_ui.backend.executor import \
    execute_action  # Import executor directly
...
async def run_agent_command(request: AgentCommandRequest):
    ...
    """
    logging_manager.log_debug("API Agent", "run_agent_command called")
    try:
        # Initialize the refactored GeminiLLM
...
Agente (responde en español):"""
        ...
        while iteration < max_iterations:
            ...
            command_match = re.search(r"execute_command:\s*(.*)", llm_response, re.IGNORECASE | re.DOTALL)
            ...
            if command_match:
                ...
                if cleaned_command.startswith("rm ") or cleaned_command.startswith("del "):
                     logging_manager.log_warning("Blocked potentially harmful command")
                     final_agent_response = "Error: Comando potencialmente peligroso bloqueado."
                     break

                logging_manager.log_debug("Agent Command", f"Comando original: {command}\nComando limpio: {cleaned_command}")
                command_to_execute = cleaned_command

                # Execute the command
                try:
                    executor_response = execute_action("execute_command", command_to_execute)
                    # Prepare the result as the next prompt for the LLM
                    current_prompt = f"System Execution Result:\n{executor_response}"
            ...
            # If no command or replace was found, treat the entire response as the final user message
                ...
                logging_manager.log_debug("API Agent", "No command/replace found, treating as final user response.")
                ...
                if "execute_command:" in final_agent_response or "replace:" in final_agent_response:
                     logging_manager.log_warning("Command/Replace potentially missed", f"Command/Replace potentially missed in final response: {final_agent_response}")
        ...
        if final_agent_response is None:
             ...
             logging_manager.log_warning("Agent loop finished without a final response (max iterations reached?).")
        ...
        logging_manager.log_error("Error executing agent command", f"Error executing agent command: {type(e).__name__} - {e}")
